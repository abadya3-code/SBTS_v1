...
import {
  ...
  bulkPasteExampleRow,
  parseBulkPaste,
} from "@shared/blindBulkPaste";
import {
  buildCertificatePdfTableSpec,
  buildTagsPdfSpec,
} from "@shared/pdfExports";
...
type ProjectStatus =
  "Active" | "Completed" | "On Hold" | "Planning" | "Final Review";
...
const defaultPhaseOwners: PhaseOwnerDraft[] = phaseOrder.map(phase => ({
  phase,
  phaseColor: "#0e7490",
  owners: [],
}));
...
function normalizePhase(value: string): BlindPhase {
  ...
  const match = phaseOrder.find(
    phase =>
      phase.toLowerCase() === normalized ||
      phase.toLowerCase().replace(/[^a-z0-9]/g, "") ===
...
function normalizeBlindType(value: string): BlindType {
  const normalized = value
    ...
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
  const match = blindTypeOptions.find(
    type => type.toLowerCase().replace(/[^a-z0-9]/g, "") === normalized
  );
...
function buildCertificateHtml(
  ...
  const certificateTitle =
    settings?.certificateTitle || "SBTS Unified Certificate Package";
  const headerCompanyName = settings?.headerCompanyName || "SBTS";
  const headerSubtitle =
    settings?.headerSubtitle || "Smart Blind Tracking System";
  const footerText = settings?.footerText || "System-generated by SBTS.";
  const logoHtml = settings?.logoUrl
    ? `<img src="${escapeHtml(settings.logoUrl)}" style="height:56px;max-width:150px;object-fit:contain" />`
    : "";
  const signatureLabels = [
    settings?.signature1Label || "Prepared By",
    settings?.signature2Label || "Reviewed By",
    settings?.signature3Label || "Approved By",
  ];
  const rows = blinds
    .map(
      (blind, index) => `
  ...
  return `<!doctype html><html><head><meta charset="utf-8"/><title>${escapeHtml(project.id)} Certificates</title><style>
    ...
    .signatures { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-top:26px; break-inside:avoid; }
  ...
  </style></head><body><section class="header"><div><div class="eyebrow">${escapeHtml(headerSubtitle)}</div><h1>${escapeHtml(certificateTitle)}</h1><p><strong>${escapeHtml(project.name)}</strong><br/>${escapeHtml(project.id)} · ${escapeHtml(project.areaCode)} · ${escapeHtml(project.areaName)}</p></div><div class="meta">${logoHtml}<br/><strong>${escapeHtml(headerCompanyName)}</strong><br/><strong>Status:</strong> ${escapeHtml(project.status)}<br/><strong>Progress:</strong> ${escapeHtml(project.progress)}%<br/><strong>Generated:</strong> ${escapeHtml(generatedAt)}</div></section><div class="cards"><div class="card">Planned<b>${metrics.plannedBlinds}</b></div><div class="card">Registered<b>${metrics.registeredBlinds}</b></div><div class="card">High Priority<b>${metrics.highPriorityBlinds}</b></div><div class="card">Critical<b>${metrics.criticalBlinds}</b></div><div class="card">Inspection Ready<b>${metrics.inspectionReadyBlinds}</b></div></div><table><thead><tr><th>#</th><th>Blind Tag</th><th>Type / Size</th><th>Phase</th><th>Priority</th><th>Phase Owner</th><th>Isolation Point</th><th>Slip Gate</th><th>Notes</th></tr></thead><tbody>${rows || "<tr><td colspan='9'>No blind records.</td></tr>"}</tbody></table><div class="signatures">${signatureLabels.map(label => `<div class="signature">${escapeHtml(label)}</div>`).join("")}</div><footer>${escapeHtml(footerText)}</footer></body></html>`;
}
...
function buildTagsHtml(
  ...
  const generatedAt = new Date().toLocaleString();
  const tagColor = tagSettings?.tagColor || "#0f172a";
  const tagWidth = Number(tagSettings?.tagWidth ?? 85);
  const tagHeight = Number(tagSettings?.tagHeight ?? 55);
  const showQR = tagSettings?.tagShowQR !== 0;
  const showLogo = tagSettings?.tagShowLogo !== 0;
  const logoUrl =
    generalSettings?.companyLogoUrl || generalSettings?.appImageUrl || "";
  const tags = blinds
    .map(
      blind => `
    <article class="tag-card">
      <div class="tag-head"><span>SBTS BLIND TAG</span><strong>${escapeHtml(blind.priority)}</strong></div>
      ${showLogo && logoUrl ? `<img class="tag-logo" src="${escapeHtml(logoUrl)}" />` : ""}
      <h2>${escapeHtml(blind.tag)}</h2>
      ${showQR ? `<div class="qr">${escapeHtml(blind.tag)}</div>` : ""}
  ...
  return `<!doctype html><html><head><meta charset="utf-8"/><title>${escapeHtml(project.id)} Tags</title><style>
    ...
    .tag-card { break-inside:avoid; border:2px solid ${tagColor}; border-radius:16px; padding:12px; width:${tagWidth}mm; min-height:${tagHeight}mm; display:flex; flex-direction:column; }
    .tag-logo { max-height:14mm; max-width:28mm; object-fit:contain; margin:8px 0; }
...
function downloadCertificatePdf(
  ...
  doc.setFontSize(16);
  doc.text(settings?.certificateTitle || spec.title, 12, 10);
  doc.setFontSize(10);
  doc.text(settings?.headerSubtitle || spec.subtitle, 12, 17);
...
function downloadTagsPdf(
  ...
  spec.pages.forEach((page, index) => {
    ...
    const tagColor = tagSettings?.tagColor || "#0e7490";
    const rgb = /^#?[0-9a-f]{6}$/i.test(tagColor)
      ? tagColor
          .replace("#", "")
          .match(/.{2}/g)!
...
function FieldGrid({
  ...
  draft: BlindDraft;
  setDraft: (draft: BlindDraft) => void;
  lockTag?: boolean;
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
            ...
            onValueChange={value => {
              ...
              setDraft({
                ...
                slipMetalForemanApproved:
                  type === "Slip Blind"
                    ? draft.slipMetalForemanApproved
                    : false,
                slipBlindMerged:
                  type === "Slip Blind" ? draft.slipBlindMerged : false,
              });
            }}
          >
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={value =>
              setDraft({ ...draft, priority: value as BlindPriority })
            }
          >
            <SelectTrigger>
              <SelectValue />
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Pump P-101 / Vessel V-204 / Line 6-FW-102"
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Train inlet header"
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            id="isolation-point"
            value={draft.isolationPoint}
            onChange={event =>
              setDraft({ ...draft, isolationPoint: event.target.value })
            }
            placeholder="Upstream ESDV-401"
          />
        </div>
      </div>
        ...
        <div className="rounded-2xl bg-cyan-50/70 p-4 ring-1 ring-cyan-100">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-extrabold text-slate-950">
                Slip Blind certificate gate
              </div>
              <p className="mt-1 text-xs leading-5 text-slate-600">
                {slipBlindGateRequired
                  ? "Slip Blind requires Foreman Metal approval and merged confirmation before saving in this project."
                  : "Slip Blind gate is currently tracked but not mandatory because the setting is disabled."}
              </p>
            </div>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <label className="flex items-center gap-3 rounded-xl bg-white p-3 text-sm font-bold text-slate-700 ring-1 ring-cyan-100">
              <Checkbox
                checked={draft.slipMetalForemanApproved}
                onCheckedChange={checked =>
                  setDraft({
                    ...draft,
                    slipMetalForemanApproved: checked === true,
                  })
                }
              />
            ...
            <label className="flex items-center gap-3 rounded-xl bg-white p-3 text-sm font-bold text-slate-700 ring-1 ring-cyan-100">
              <Checkbox
                checked={draft.slipBlindMerged}
                onCheckedChange={checked =>
                  setDraft({ ...draft, slipBlindMerged: checked === true })
                }
              />
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Execution notes, safety holds, torque reference, or certificate comments"
        />
      </div>
    </div>
  );
...
export default function ProjectDetail() {
  const [isAreaProjectRoute, areaProjectParams] = useRoute(
    "/areas/:areaId/projects/:projectId"
  );
  const [, projectParams] = useRoute("/projects/:projectId");
  const projectId = isAreaProjectRoute
    ? areaProjectParams?.projectId
  ...
  const [editingDraft, setEditingDraft] =
    useState<BlindDraft>(defaultBlindDraft);
  const [bulkText, setBulkText] = useState("");
  const [settingsDraft, setSettingsDraft] =
    useState<PhaseOwnerDraft[]>(defaultPhaseOwners);
  const [slipBlindGateRequiredDraft, setSlipBlindGateRequiredDraft] =
    useState(true);

  const detailQuery = trpc.projects.detail.useQuery(
    { id: projectId ?? "" },
    { enabled: Boolean(projectId) }
  );
  const certSettingsQuery = trpc.settings.certificate.get.useQuery(undefined, {
    staleTime: 60_000,
  });
  const tagSettingsQuery = trpc.settings.defaultTag.get.useQuery(undefined, {
    staleTime: 60_000,
  });
  const generalSettingsQuery = trpc.settings.general.get.useQuery(undefined, {
    staleTime: 60_000,
  });
  const assignableUsersQuery = trpc.projects.settings.assignableUsers.useQuery(
    undefined,
  ...
  const registeredPercentage =
    metrics && metrics.plannedBlinds > 0
      ? Math.round((metrics.registeredBlinds / metrics.plannedBlinds) * 100)
      : 0;

  const phaseOwners = useMemo(
    () =>
      detail?.settings.phaseOwners ??
    ...
    [detail?.settings.phaseOwners, projectId]
  );
  const slipBlindGateRequired = detail?.settings.slipBlindGateRequired ?? true;
  const phaseOwnerByPhase = useMemo(
    () => new Map(phaseOwners.map(owner => [owner.phase, owner])),
    [phaseOwners]
  );
  ...
  const canEditPhase = (phase: BlindPhase) => {
    ...
    const userTokens = [user?.openId, user?.name, user?.email]
      .map(token => token?.trim().toLowerCase())
      .filter((token): token is string => Boolean(token));
    return owners.some(assignee =>
      [assignee.openId, assignee.name, assignee.email ?? ""]
        .map(token => token.trim().toLowerCase())
        .some(token => token && userTokens.includes(token))
    );
  };
  ...
  const bulkAddMutation = trpc.projects.bulkAddBlinds.useMutation({
    onSuccess: async result => {
      toast.success(
        `${result.count} blind records imported from the pasted table.`
      );
      setBulkOpen(false);
      setBulkText("");
      await invalidateProject();
  ...
  const openSettings = () => {
    setSettingsDraft(
      phaseOwners.map(owner => ({
        phase: owner.phase as BlindPhase,
        phaseColor: owner.phaseColor ?? "#0e7490",
        owners: owner.owners ?? [],
      }))
    );
  ...
  const submitSingle = () => {
    ...
    if (!canAddCurrentBlind) {
      toast.error(
        "You can only add blinds when you are assigned to the preparation phase."
      );
      return;
    }
    if (
      slipBlindGateRequired &&
      blindDraft.type === "Slip Blind" &&
      (!blindDraft.slipMetalForemanApproved || !blindDraft.slipBlindMerged)
    ) {
      toast.error(
        "Slip Blind requires Foreman Metal approval and merged confirmation while the project setting is active."
      );
      return;
    }
  ...
  const submitBulk = () => {
    ...
    if (!canImportBulk) {
      toast.error(
        "Bulk import is limited to users assigned to the preparation phase."
      );
      return;
    }
    if (
      slipBlindGateRequired &&
      bulkPreview.parsed.some(
        blind =>
          blind.type === "Slip Blind" &&
          (!blind.slipMetalForemanApproved || !blind.slipBlindMerged)
      )
    ) {
      toast.error(
        "Every Slip Blind row must include Foreman Metal approval and merged confirmation while the project setting is active."
      );
      return;
    }
  ...
  const submitEdit = () => {
    ...
    if (!canSaveEditingBlind) {
      toast.error(
        "You cannot move or update this blind into a phase owned by another user."
      );
      return;
    }
    if (
      slipBlindGateRequired &&
      editingDraft.type === "Slip Blind" &&
      (!editingDraft.slipMetalForemanApproved || !editingDraft.slipBlindMerged)
    ) {
      toast.error(
        "Slip Blind requires Foreman Metal approval and merged confirmation while the project setting is active."
      );
  ...
  const addAssigneeToPhase = (
    phase: BlindPhase,
    assignee: PhaseAssigneeDraft
  ) => {
    setSettingsDraft(current =>
      current.map(item => {
        if (item.phase !== phase) return item;
        if (item.owners.some(owner => owner.openId === assignee.openId))
          return item;
        return {
          ...item,
          owners: [
    ...
    setSettingsDraft(current =>
      current.map(item =>
        item.phase === phase
          ? {
              ...item,
              owners: item.owners.filter(owner => owner.openId !== openId),
            }
          : item
      )
    );
  ...
  const submitSettings = () => {
    ...
    settingsMutation.mutate({
      projectId,
      slipBlindGateRequired: slipBlindGateRequiredDraft,
      phaseOwners: settingsDraft.map(item => ({
        phase: item.phase,
  ...
  const exportCertificates = () => {
    ...
    downloadCertificatePdf(
      project,
      blinds as ExportBlind[],
      metrics,
      certSettingsQuery.data
    );
  ...
  const exportTags = () => {
    if (!project) return;
    downloadTagsPdf(project, blinds as ExportBlind[], tagSettingsQuery.data);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={
          project
            ? `${project.id} · ${project.areaCode} · ${project.areaName}. Review linked blind records, priority exposure, phase owners, and export-ready packages from one operational view.`
            : "Load a project to review its linked blind records, execution status, and readiness indicators."
        }
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href={backHref}
              className="inline-flex items-center gap-2 rounded-2xl bg-white px-4 py-2.5 text-sm font-extrabold text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:bg-slate-50"
            >
              <ArrowLeft className="h-4 w-4" />{" "}
              {areaId ? "Area projects" : "Projects"}
            </Link>
            <Button
              type="button"
              onClick={() => {
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-red-700">
                  The project detail API did not respond successfully. Retry
                  after confirming database connectivity.
                </p>
              </div>
            </div>
            <button
              onClick={() => detailQuery.refetch()}
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-red-700 px-4 py-2.5 text-sm font-extrabold text-white shadow-sm transition hover:bg-red-800"
            >
      ...
      {!detailQuery.isLoading && !detailQuery.isError && !detail && (
        <div className="sbts-card border-amber-100 bg-amber-50/80 p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-amber-800">
                  The requested project identifier is invalid, archived, or no
                  longer exists. Return to the project list and select an active
                  record.
                </p>
              </div>
            </div>
            <Link
              href={backHref}
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-700 px-4 py-2.5 text-sm font-extrabold text-white shadow-sm transition hover:bg-amber-800"
            >
        ...
        <>
          ...
          <WorkflowPhases
            phases={phaseOrder.map(phase => ({
              ...
              progress: Math.round(
                ((metrics.phaseCounts[phase] ?? 0) /
                  Math.max(1, metrics.registeredBlinds)) *
                  100
              ),
          ...
          <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h2 className="mt-2 text-xl font-extrabold text-slate-950">
                    Certificates and tags package
                  </h2>
                  <p className="mt-2 text-sm font-medium leading-6 text-slate-600">
                    Use one consolidated package when sending certificates or
                    blind tags to the printer instead of saving each item
                    separately.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    <FileDown className="h-4 w-4" /> Export certificates
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
                    ...
                    onClick={exportTags}
                  >
                    <Tags className="h-4 w-4" /> Export tags
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
            ...
            <div className="sbts-card p-5">
              ...
              {!canManageSettings && (
                <div className="mt-4 rounded-2xl bg-amber-50 p-3 text-xs font-bold leading-5 text-amber-800 ring-1 ring-amber-100">
                  Only administrators can change phase-owner settings. Other
                  users can view assignments and update only their assigned
                  phase.
                </div>
              )}
            </div>
      ...
      <Dialog open={singleOpen} onOpenChange={setSingleOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Add a single blind</DialogTitle>
            <DialogDescription>
              Create one independent blind record linked to this project.
            </DialogDescription>
          </DialogHeader>
          <FieldGrid
            draft={blindDraft}
            setDraft={setBlindDraft}
            slipBlindGateRequired={slipBlindGateRequired}
          />
          {!canAddCurrentBlind && (
            <div className="rounded-2xl bg-amber-50 p-3 text-sm font-bold text-amber-800 ring-1 ring-amber-100">
              This phase is assigned to another owner, so your current account
              can only view it.
            </div>
          )}
          <DialogFooter>
            ...
            <Button
              ...
              disabled={addBlindMutation.isPending || !canAddCurrentBlind}
            >
              Save blind
            </Button>
          </DialogFooter>
      ...
      <Dialog open={editingOpen} onOpenChange={setEditingOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Edit blind record</DialogTitle>
            <DialogDescription>
              Only configured phase assignees or administrators can update the
              current workflow phase. The phase itself remains controlled by
              workflow progress, not by Add Blind.
            </DialogDescription>
          </DialogHeader>
          <FieldGrid
            draft={editingDraft}
          ...
          {!canSaveEditingBlind && (
            <div className="rounded-2xl bg-amber-50 p-3 text-sm font-bold text-amber-800 ring-1 ring-amber-100">
              This workflow phase is assigned to another person. Ask an
              administrator to update Project phase owners if you need access.
            </div>
          )}
          <DialogFooter>
            <Button
            ...
            <Button
              type="button"
              onClick={submitEdit}
              disabled={updateBlindMutation.isPending || !canSaveEditingBlind}
            >
              Update blind
            </Button>
          </DialogFooter>
      ...
      <Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,1180px)] max-w-none overflow-hidden p-0">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Import multiple blind records using a controlled Excel paste
              workflow. Keep the column order below so the system can validate
              each row before import.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
            <section className="space-y-4">
              <div className="rounded-2xl border border-cyan-100 bg-cyan-50/70 p-4 dark:border-cyan-900/40 dark:bg-cyan-950/20">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <div className="text-xs font-extrabold uppercase tracking-[0.18em] text-cyan-700 dark:text-cyan-300">
                      Step 1 · Excel format
                    </div>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-300">
                      Copy rows directly from Excel. One row equals one blind.
                      Use Yes/No for the Slip Blind gate fields.
                    </p>
                  </div>
                  <Button
                    type="button"
                    variant="outline"
                    className="shrink-0 rounded-xl bg-white dark:bg-slate-950"
                    onClick={() => setBulkText(bulkPasteExampleRow)}
                  >
                    Load example
                  </Button>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                  {bulkPasteColumns.map((column, index) => (
                    <div
                      key={column}
                      className="rounded-xl border border-cyan-100 bg-white p-2 text-xs dark:border-cyan-900/40 dark:bg-slate-950"
                    >
                ...
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs font-semibold text-slate-500">
                      Paste from Excel, review the preview, then import.
                    </p>
                  </div>
                  <Badge variant="outline">
                    {bulkPreview.parsed.length} valid
                  </Badge>
                </div>
                <Textarea
                  ...
                  placeholder={bulkPasteExampleRow}
                />
              </div>
            </section>
            ...
            <section className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm font-extrabold text-slate-950 dark:text-white">
                      {bulkPreview.parsed.length} valid rows ·{" "}
                      {bulkPreview.errors.length} issues
                    </div>
                  </div>
                ...
                {bulkPreview.errors.length > 0 && (
                  <div className="mt-4 max-h-28 overflow-auto rounded-xl bg-red-50 p-3 text-xs font-semibold text-red-700 ring-1 ring-red-100 dark:bg-red-950/20 dark:text-red-300 dark:ring-red-900/40">
                    {bulkPreview.errors.slice(0, 6).map(error => (
                      <div key={error}>• {error}</div>
                    ))}
                  </div>
                )}

                {bulkPreview.parsed.length > 0 && !canImportBulk && (
                  <div className="mt-4 rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-800 ring-1 ring-amber-100 dark:bg-amber-950/20 dark:text-amber-300 dark:ring-amber-900/40">
                    Bulk creation starts every blind in the preparation workflow
                    phase. Only administrators or assigned preparation-phase
                    users can import new rows.
                  </div>
                )}

                {bulkPreview.parsed.length > 0 ? (
                  <div className="mt-4 max-h-80 overflow-auto rounded-xl border border-slate-100 dark:border-slate-800">
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {bulkPreview.parsed.slice(0, 10).map(blind => (
                          <TableRow key={blind.tag}>
                            <TableCell className="font-bold">
                              {blind.tag}
                            </TableCell>
                            <TableCell>{blind.type}</TableCell>
                            <TableCell>{blind.size}</TableCell>
                            <TableCell>{blind.rate || "—"}</TableCell>
                            <TableCell>{blind.equipment || "—"}</TableCell>
                            <TableCell>
                              {blind.type === "Slip Blind"
                                ? `${blind.slipMetalForemanApproved ? "Foreman OK" : "Foreman pending"} / ${blind.slipBlindMerged ? "Merged" : "Not merged"}`
                                : "N/A"}
                            </TableCell>
                          </TableRow>
                        ))}
                  ...
                  <div className="mt-4 rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm font-semibold text-slate-500 dark:border-slate-800">
                    Paste Excel rows to preview validated blinds here.
                  </div>
                )}
              </div>
            </section>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="max-h-[92vh] w-[min(96vw,1200px)] max-w-none overflow-hidden p-0">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Assign project workers to phases, set the phase colors used in
              Execution Brief, and control whether Slip Blind approvals are
              mandatory.
            </DialogDescription>
          </DialogHeader>
          <div className="mb-4 rounded-2xl bg-cyan-50/70 p-4 ring-1 ring-cyan-100">
            <label className="flex items-start gap-3">
              <Checkbox
                checked={slipBlindGateRequiredDraft}
                disabled={!canManageSettings}
                onCheckedChange={checked =>
                  setSlipBlindGateRequiredDraft(checked === true)
                }
              />
              <span>
                <span className="block text-sm font-extrabold text-slate-950">
                  Require Slip Blind Foreman gate
                </span>
                <span className="mt-1 block text-xs font-bold leading-5 text-slate-600">
                  When active, any Slip Blind must have Foreman Metal approval
                  and merge confirmation before saving.
                </span>
              </span>
            </label>
          </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {settingsDraft.map(owner => {
              const availableForPhase = assignableUsers.filter(
                candidate =>
                  !owner.owners.some(
                    selected => selected.openId === candidate.openId
                  )
              );
              return (
                <div
                ...
                >
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <label className="mt-4 inline-flex items-center gap-3 rounded-2xl bg-slate-50 px-3 py-2 text-xs font-extrabold text-slate-600 ring-1 ring-slate-200">
                        ...
                        <input
                          type="color"
                          value={owner.phaseColor}
                          disabled={!canManageSettings}
                          onChange={event =>
                            setSettingsDraft(current =>
                              current.map(item =>
                                item.phase === owner.phase
                                  ? { ...item, phaseColor: event.target.value }
                                  : item
                              )
                            )
                          }
                          className="h-9 w-14 rounded-xl border border-slate-200 bg-white p-1"
                        />
                      </label>
                    </div>
                    <Select
                      disabled={
                        !canManageSettings || availableForPhase.length === 0
                      }
                      onValueChange={openId => {
                        const assignee = assignableUsers.find(
                          candidate => candidate.openId === openId
                        );
                    ...
                    >
                      <SelectTrigger className="w-full bg-white lg:w-72">
                        <SelectValue
                          placeholder={
                            availableForPhase.length
                              ? "Add signed-in person"
                              : "No more users"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                        {availableForPhase.map(candidate => (
                          <SelectItem
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {owner.owners.length === 0 && (
                      <div className="rounded-full bg-white px-3 py-2 text-xs font-bold text-slate-500 ring-1 ring-slate-200">
                        No dedicated assignee yet
                      </div>
                    )}
                    {owner.owners.map(assignee => (
                      <div
                        key={assignee.openId}
                        className="flex items-center gap-2 rounded-full bg-white px-2 py-1.5 ring-1 ring-slate-200"
                      >
                        <Avatar className="h-8 w-8">
                          ...
                          <AvatarFallback className="text-[10px] font-extrabold">
                            {initials(assignee.name)}
                          </AvatarFallback>
                        </Avatar>
                        <div className="min-w-0">
                          <div className="max-w-40 truncate text-xs font-extrabold text-slate-950">
                            {assignee.name}
                          </div>
                          {assignee.email && (
                            <div className="max-w-40 truncate text-[10px] font-semibold text-slate-500">
                              {assignee.email}
                            </div>
                          )}
                        </div>
          ...
          <DialogFooter className="mt-6">
            ...
            <Button
              ...
              disabled={settingsMutation.isPending || !canManageSettings}
            >
              Save settings
            </Button>
          </DialogFooter>
